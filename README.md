...
Скрипт лише переписує `lib/catalog-data.ts` і нічого сам не комітить і не
деплоїть — після `--write` звичайний огляд diff, `npm run typecheck`,
`next build` і коміт, як і для будь-якої іншої зміни каталогу.
